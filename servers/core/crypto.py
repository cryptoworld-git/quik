...
class QuiCrypt(object):
    ...
    def keyLoader(self, private, passwd=None):
        '''Load a key and generate the public pair if only the private is avaliable'''
        privkey = RSA.importKey(private, passphrase=passwd)  # Import the Private Key, optionally with password if it's encrypted
        pubkey = privkey.publickey().exportKey("OpenSSH")  # Export the public key in OpenSSH format 
